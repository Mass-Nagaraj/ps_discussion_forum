...
// a={ email:email, id:post_id, SubReplyId: SubReplyId }
  ...
  useEffect(()=>{
   ...
   const toggleLike = (event) => {
    ...
    if(!liked) {
      ...
      axios.post("http://localhost:2000/delLike2",ReplyType)
      .then((res)=>{
          console.log("Removed form like table..",res.data)
      }).catch((err)=>console.log(err));
  ...
  const toggleBmark = () => {
    ...
    if(!marked) {
      ...
      // setMarked((marked)=>(marked-1));
      axios.post("http://localhost:2000/delSave2",ReplyType)
      .then((res)=>{
          console.log("Removed form dave table..")
      }).catch((err)=>console.log(err));
