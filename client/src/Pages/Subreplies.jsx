...
import ReplyFrom2 from './ReplyForm2';
...
if(SubReplyId==null) {
    ...
    axios.post("http://localhost:2000/getSubreplies1",{MainReplyId:MainReplyId}
    ).then((res)=>{
    ...
    axios.post("http://localhost:2000/getSubreplies1",{SubReplyId:SubReplyId}
).then((res)=>{
...
function EditSubReply(e,id) {
    e.preventDefault();
    axios.post('http://localhost:2000/EditSubReply',{id:id})
    .then((res)=>{
...
function handleSubmit1(event){
    ...
    event.preventDefault();
        ...
        axios.post('http://localhost:2000/Editsubreplies',formData1)
            .then((res)=>{
            console.log(res.data)
            setReply('')
            setFile('')
                                ...
                                <div className='secrepexample'>
                                    ...
                                    <div className='secrephead'>
                                        ...
                                        <div className='secreppara'>
                                            ...
                                            <Likes  email={email} post_id={post_id} SubReplyId={value.id}/>
                                           
                                            {(value.from_email== email) ?
                                                    <div className='actions'>
                                                        <div className='edit' >
                                                            <button type="button" style={{"backgroundColor":"white"}} onClick={(e)=>EditSubReply(e,value.id)} ><FaPen className='edit-icon'/></button>
                                                        </div>
                                                        <div className='delete'>
                                                        <Deletealertbox  style={{"backgroundColor":"white"}}  senddd={(data)=> {
                                                        if(data=="yes") {
                                                            deltReply(value.id);
                                                        }
                                                        }}/> {/*onClick={deltReply} */}
                            ...
                            <form onSubmit={handleSubmit1}>
                                    <div className={`drawer ${isOpen ? 'open' : ''}`}>
                                            <div className="drawer-content">
                                                    ...
                                                    <h2> {question1}<br/> Reply to {value.username}</h2>
                                            ...
                                            <div className="drawer-footer">
                                                <button type='button' className="cancel-btn" onClick={closeDrawer}>
                                                      Cancel
                                                </button>
